...
function Completed ({ data }) {
  return (
    <div className={styles.sales}>
      <div className={styles.title}>展馆近期累计用电量</div>
      <ResponsiveContainer minHeight={360}>
        <AreaChart data={data}>
          ...
          />
          <Area type="monotone" dataKey="1号厅" stroke={color.grass} fill={color.grass} strokeWidth={2} dot={{ fill: '#fff' }} activeDot={{ r: 5, fill: '#fff', stroke: color.green }} />
          <Area type="monotone" dataKey="2号厅" stroke={color.sky} fill={color.sky} strokeWidth={2} dot={{ fill: '#fff' }} activeDot={{ r: 5, fill: '#fff', stroke: color.blue }} />
        </AreaChart>
      </ResponsiveContainer>
    </div>
  )
