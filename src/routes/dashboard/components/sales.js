...
function Sales ({ data }) {
  return (
    <div className={styles.sales}>
      <div className={styles.title}>热门展区参观人数统计</div>
      <ResponsiveContainer minHeight={360}>
        <LineChart data={data}>
          ...
          />
          <Line type="monotone" dataKey="历史文化厅" stroke={color.purple} strokeWidth={3} dot={{ fill: color.purple }} activeDot={{ r: 5, strokeWidth: 0 }} />
          <Line type="monotone" dataKey="规划战略厅" stroke={color.red} strokeWidth={3} dot={{ fill: color.red }} activeDot={{ r: 5, strokeWidth: 0 }} />
          <Line type="monotone" dataKey="城市概况厅" stroke={color.green} strokeWidth={3} dot={{ fill: color.green }} activeDot={{ r: 5, strokeWidth: 0 }} />
        </LineChart>
      </ResponsiveContainer>
    </div>
  )
